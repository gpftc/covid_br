...
def get_data_covid(city:str,state:str) -> str:
    ...
    date_file_upload = get_modify_day_file(path_verify = path_cache_dir+'date_update_verification')
    log('verifycatting the fileDateUpdate')
    if vf['update']:
        log('fileDateUpdate is update!')
        if os.path.isfile(file_content) and (date_file_content==date_file_upload):
